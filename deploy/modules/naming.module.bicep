 ...
 */
 ...
 output names object = {
   ...
   cognitiveSearch: {
    name: substring(replace(nameTemplate, placeholder, 'srch'), 0, min(length(replace(nameTemplate, placeholder, 'srch')), 64))
    nameUnique: substring(replace(nameUniqueTemplate, placeholder, 'srch'), 0, min(length(replace(nameUniqueTemplate, placeholder, 'srch')), 64))
    slug: 'srch'
  }
   cognitiveAccount: {
